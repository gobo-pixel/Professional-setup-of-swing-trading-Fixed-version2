...
class BacktestEngine:
    ...
    def run(
        ...
        for step in range(min_history, total_steps):
            broker_status = {"status": "ONLINE", "mode": "BACKTEST"}
            market_state = {"max_trade_candidates": max_candidates_per_day, "max_watchlist": 50}
            ...
            for candidate in candidates:
                ...
                if candidate.action == "BUY":
                    if candidate.symbol not in portfolio.state.open_positions:
